...
import { isWorkingDay } from '../utils/jobCalendar';
import css from './MoneyPage.module.css';

export const DEFAULT_MONEY_STATE: MoneyState = {
    sekTotal: 125000,
    jpyTotal: 0,
    monthlyIncomeSek: 34000,
    monthlyExpensesSek: 18000,
    ...
    seMunicipalTaxRatePct: 32,
    seStateTaxThresholdSek: 625_800,
    seStateTaxRatePct: 20,
    sePensionRatePct: 7,
    seBasicDeductionSek: 16_000,
    seJobTaxCreditMaxSek: 37_000,
    sePublicServiceFeeRatePct: 1,
    sePublicServiceFeeMaxSek: 1_600,
    seBurialFeeRatePct: 0.29,
    seChurchFeeRatePct: 1.03,
    seIncludeChurchFee: false,
    jpHealthInsuranceRatePct: 4.95,
    jpPensionRatePct: 9.15,
    jpUnemploymentRatePct: 0.6,
    jpLongTermCareRatePct: 0.91,
    jpAge: 30,
    jpResidentTaxRatePct: 10,
    jpResidentPerCapitaAnnualJpy: 5_000,
    jpBasicDeductionNationalJpy: 480_000,
    jpBasicDeductionResidentJpy: 430_000,
    jpDependentDeductionAnnualJpy: 0,
    csnEnabled: false,
    csnRepaymentModel: 'annuity',
    csnOutstandingPrincipalSek: 0,
    csnAnnualInterestRatePct: 1.23,
    csnAdminFeeAnnualSek: 150,
    csnRepaymentYears: 25,
...
export const normalizeMoneyState = (state?: Partial<MoneyState>): MoneyStateNormalized => {
    ...
    return {
        ...
        seMunicipalTaxRatePct: state?.seMunicipalTaxRatePct ?? DEFAULT_MONEY_STATE.seMunicipalTaxRatePct ?? 32,
        seStateTaxThresholdSek: state?.seStateTaxThresholdSek ?? DEFAULT_MONEY_STATE.seStateTaxThresholdSek ?? 625_800,
        seStateTaxRatePct: state?.seStateTaxRatePct ?? DEFAULT_MONEY_STATE.seStateTaxRatePct ?? 20,
        sePensionRatePct: state?.sePensionRatePct ?? DEFAULT_MONEY_STATE.sePensionRatePct ?? 7,
        seBasicDeductionSek: state?.seBasicDeductionSek ?? DEFAULT_MONEY_STATE.seBasicDeductionSek ?? 16_000,
        seJobTaxCreditMaxSek: state?.seJobTaxCreditMaxSek ?? DEFAULT_MONEY_STATE.seJobTaxCreditMaxSek ?? 37_000,
        sePublicServiceFeeRatePct: state?.sePublicServiceFeeRatePct ?? DEFAULT_MONEY_STATE.sePublicServiceFeeRatePct ?? 1,
        sePublicServiceFeeMaxSek: state?.sePublicServiceFeeMaxSek ?? DEFAULT_MONEY_STATE.sePublicServiceFeeMaxSek ?? 1_600,
        seBurialFeeRatePct: state?.seBurialFeeRatePct ?? DEFAULT_MONEY_STATE.seBurialFeeRatePct ?? 0.29,
        seChurchFeeRatePct: state?.seChurchFeeRatePct ?? DEFAULT_MONEY_STATE.seChurchFeeRatePct ?? 1.03,
        seIncludeChurchFee: state?.seIncludeChurchFee ?? DEFAULT_MONEY_STATE.seIncludeChurchFee ?? false,
        jpHealthInsuranceRatePct: state?.jpHealthInsuranceRatePct ?? DEFAULT_MONEY_STATE.jpHealthInsuranceRatePct ?? 4.95,
        jpPensionRatePct: state?.jpPensionRatePct ?? DEFAULT_MONEY_STATE.jpPensionRatePct ?? 9.15,
        jpUnemploymentRatePct: state?.jpUnemploymentRatePct ?? DEFAULT_MONEY_STATE.jpUnemploymentRatePct ?? 0.6,
        jpLongTermCareRatePct: state?.jpLongTermCareRatePct ?? DEFAULT_MONEY_STATE.jpLongTermCareRatePct ?? 0.91,
        jpAge: state?.jpAge ?? DEFAULT_MONEY_STATE.jpAge ?? 30,
        jpResidentTaxRatePct: state?.jpResidentTaxRatePct ?? DEFAULT_MONEY_STATE.jpResidentTaxRatePct ?? 10,
        jpResidentPerCapitaAnnualJpy: state?.jpResidentPerCapitaAnnualJpy ?? DEFAULT_MONEY_STATE.jpResidentPerCapitaAnnualJpy ?? 5_000,
        jpBasicDeductionNationalJpy: state?.jpBasicDeductionNationalJpy ?? DEFAULT_MONEY_STATE.jpBasicDeductionNationalJpy ?? 480_000,
        jpBasicDeductionResidentJpy: state?.jpBasicDeductionResidentJpy ?? DEFAULT_MONEY_STATE.jpBasicDeductionResidentJpy ?? 430_000,
        ...
        csnAnnualInterestRatePct: state?.csnAnnualInterestRatePct ?? DEFAULT_MONEY_STATE.csnAnnualInterestRatePct ?? 1.23,
        csnAdminFeeAnnualSek: state?.csnAdminFeeAnnualSek ?? DEFAULT_MONEY_STATE.csnAdminFeeAnnualSek ?? 150,
        csnRepaymentYears: state?.csnRepaymentYears ?? DEFAULT_MONEY_STATE.csnRepaymentYears ?? 25,
        ...
        showTaxProfile: state?.showTaxProfile ?? DEFAULT_MONEY_STATE.showTaxProfile ?? true,
...
export const getExpectedJobSalaryForMonth = (monthDate: Date, jobState: JobCalendarState): number => {
    ...
    const workingHours = workingDays * jobState.workingHoursPerDay;
